//scrapes bike race info data for Paris Roubaix interactive
...
function getBikesAfter2014(body, year) {
  ...
  //'mod' sets where to find an element that is inconsistant located
	var mod = 0;
	if (year == 2016) { mod = 2; }
		...
		$('div.content table').eq(mod).find('tr').each(function(j, elem){
				...
				if (year == 2016) {
				  var lastSpace = td.eq(1).text().trim().lastIndexOf(' ');
				  var name = td.eq(1).text().trim();
				  var fName = name.slice(lastSpace+1,name.length).toLowerCase().trim();
				  var lName = name.slice(0,lastSpace).toLowerCase().trim();
				  //fName = fName[0].toUpperCase() + fName.
